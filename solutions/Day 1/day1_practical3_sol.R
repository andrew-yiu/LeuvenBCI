...
set.seed(1)
nhanes <- read.table(file = "~/Downloads/LeuvenBCI-main/data/nhanes_sub.csv") 
...
M <- 3000 ## number of forward steps in predictive resampling
...
for (m in 1:M) {
  x_vec[m] <- sample(c(x,x_vec[m-1]), size = 1)
  prop[m] <- length(which(x_vec == 65))/m ## track proportion of predicted x-values that equal 65
}

## plot line graph to check convergence
plot(1:M, prop, type = "l", xlab = "Forward step")
...
############################################
### Include smoking status as confounder ###
############################################

### As described in the solutions document, we consider adding
### smoking status as an additional confounder and compare the results
### to above. 

n <- length(y)
ate_samp_new <- numeric(B) # create new vector to store ATE values for the augmented model

y <- nhanes$years.lived.since.1971.up.to.1992
t <- nhanes$physically.active
x <- nhanes$age.at.interview
s <- nhanes$smoking.status # extract smoking status from dataset

## Split s into indicators for the current and former smokers
sc <- as.numeric(s == "Current")
sf <- as.numeric(s == "Former")

B <- 500 ## number of posterior samples
M <- 2000 ## number of forward steps in predictive resampling

### compute initial parameter values
dat <- data.frame(x = x, y = y, t = t, sc = sc, sf = sf)
lin_fit <- lm(y ~ t + x + sc + sf, data = dat)
coef <- lin_fit$coefficients
w <- cbind(rep(1, length(x)), t, x, sc, sf)
s2 <- (1/(n-5))*sum((y-w %*% as.matrix(coef))^2) # change degrees of freedom to account for additional covariate dimensions
V <- solve(t(w) %*% w)
...
for (b in 1:B) {
  ...
  dir_wts <- rexp(n)
  dir_wts <- dir_wts/sum(dir_wts)
  bb_count <- sample(1:n, size = M, replace = TRUE, prob = dir_wts)
  xpred <- x[bb_count]
  # xpred <- rnorm(M, mean = 60, sd = 8) 
  xpred <- c(x, xpred)
  scpred <- sc[bb_count]
  ...
  ### Combine t and all covariates into w
  wpred <- cbind(rep(1,n+M), tpred, xpred, scpred, sfpred) # vector of 1's is for the intercept
  for (j in 1:M) {
    ### draw new y value from generalized t-distribution
    ynew <- rt(1,df = N-5)
    ynew <- sum(as.vector(wpred[n+j,])*coefpred) + ynew * sqrt(s2pred*(1+t(wpred[n+j,]) %*% Vpred %*% wpred[n+j,]))
    ...
    N <- N+1
    coefpred <- tcrossprod(Vpred, wpred[1:(n+j),]) %*% as.matrix(ypred)
    s2pred <- (1/(N-5))*sum((ypred-wpred[1:(n+j),] %*% as.matrix(coefpred))^2)
    
  }
  ### compute ATE via difference-in-means estimator for the imputed target trial population
  ysim <- ypred[(n+1):(n+M)] ### imputed outcomes
  tsim <- wpred[(n+1):(n+M), 2] ### imputed treatments
  ate_samp_new[b] <- mean(ysim[which(tsim==1)]) - mean(ysim[which(tsim==0)])
}
## compare densities with and without smoking status
plot(density(ate_samp), main = "Posterior for the ATE", xlab = "ATE (years)", type = "l", col = "red")
lines(density(ate_samp_new), col = "green")
legend(-0.3, 0.6, legend=c("Without smoking", "With smoking"),
